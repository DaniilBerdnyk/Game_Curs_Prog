...
public class NetworkDiscovery
{
    ...
    private const string DiscoveryMessage = "DISCOVER_HOST";

    public static async Task<IPAddress> DiscoverHostAsync()
    {
    ...
    public static async Task StartHostDiscoveryAsync()
    {
        using (var udpClient = new UdpClient(DiscoveryPort))
        {
            while (true)
            {
                ...
                {
                    var response = Encoding.UTF8.GetBytes("HOST_HERE");
